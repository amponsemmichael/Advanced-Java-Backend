...
import com.amponsem.model.Nurse;
import com.amponsem.services.NurseService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;
...
public class NurseController {
    ...
    public ResponseEntity<Nurse> updateNurse(@PathVariable Long id, @RequestBody Nurse nurseDetails) {
        ...
        return updatedNurse.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity<Void> deleteNurse(@PathVariable Long id) {
        boolean isDeleted = nurseService.deleteNurse(id);
